...
### Predictor Matrix
X <- df_model2 |>
  dplyr::select(-LocationAbbr, -YearStart, -PercentOverall) |>
  as.matrix()
...
```{r}
### Age 18 - 24 has the smallest SD
### Other age demographics have larger between state variance
df_wide |> 
  summarize(across(starts_with("Age"), ~ sd(.x, na.rm = TRUE)))
```

```{r}
### Create Lasso for comparison
...
ridge_df <- data.frame(
  variable = ridge_coef@Dimnames[[1]],
...
lasso_df <- data.frame(
  variable = lasso_coef@Dimnames[[1]],
...
```{r}
### Comparison
coef_compare <- ridge_df |>
  full_join(lasso_df, by = "variable") |>
...
```{r}
coef_compare |> filter(lasso != 0) |> arrange(desc(abs(lasso)))
```
...
coef_table <- coef_compare |>
  mutate(
    ridge = round(ridge, 3),
    lasso = round(lasso, 3),
    selected_lasso = ifelse(lasso != 0, "Yes", "No")
  ) |>
  arrange(desc(abs(ridge)))  
...
cv_lasso$cvm      
cv_lasso$lambda 
best_mse_lasso <- cv_lasso$cvm[cv_lasso$lambda == best_lambda_lasso]
best_mse_lasso
